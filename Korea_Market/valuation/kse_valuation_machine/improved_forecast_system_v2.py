...
def find_and_import_modules():
    """
    stock_forecast/DATA 폴더와 ticker_list.py를 자동으로 찾아 import
    노트북과 데스크탑 등 다른 환경에서도 자동으로 경로 탐색
    """
    try:
    ...
    print(f"✅ stock_forecast 경로 발견: {stock_forecast_path}")
    
    # 2. ticker_list.py 찾기 (여러 위치 탐색)
    ticker_list_paths = [
        stock_forecast_path / "DATA" / "ticker_list.py",  # DATA 폴더 내
        stock_forecast_path / "ticker_list.py",            # stock_forecast 폴더 내
        current_path.parent / "DATA" / "ticker_list.py",   # 현재 경로의 DATA 폴더
        current_path.parent / "ticker_list.py",            # 현재 경로의 부모 폴더
        current_path / "ticker_list.py",                   # 현재 경로
    ]
    
    ticker_list_found = None
    for path in ticker_list_paths:
        if path.exists():
            ticker_list_found = path
            ticker_list_dir = str(path.parent)
            if ticker_list_dir not in sys.path:
                sys.path.insert(0, ticker_list_dir)
            print(f"✅ ticker_list.py 발견: {path}")
            break
    
    if ticker_list_found is None:
        print("⚠️  ticker_list.py를 찾을 수 없습니다.")
        print("다음 위치에 ticker_list.py 파일을 생성하세요:")
        print(f"   {stock_forecast_path / 'DATA' / 'ticker_list.py'}")
        return None
    
    # 3. 모듈 import
    try:
        import ticker_list
        print(f"✅ ticker_list 모듈 import 성공")
        return ticker_list
    except ImportError as e:
        print(f"❌ ticker_list 모듈 import 실패: {e}")
        return None


# 경로 탐색 및 모듈 import 실행
print("=" * 60)
print("경로 탐색 및 모듈 로딩 중...")
print("=" * 60)
ticker_list_module = find_and_import_modules()
...
def process_multiple_tickers(ticker_list, db_info, table_name='Korea_company_valuation_ver2', error_log_file='error_log.txt'):
    """여러 티커를 순차적으로 처리"""
    success_count = 0
    error_tickers = []
...
def load_ticker_list(group='all'):
    """
    ticker_list.py에서 티커 리스트를 로드
    
    Parameters:
    -----------
    group : str
        'all', 'test', 'semiconductor', 'it', 'auto', 'bio', 'chemical' 등
    
    Returns:
    --------
    list : 티커 딕셔너리 리스트
    """
    if ticker_list_module is None:
        print("❌ ticker_list 모듈을 사용할 수 없습니다.")
        print("수동으로 티커 리스트를 입력하세요.")
        return []
    
    try:
        ticker_list = ticker_list_module.get_ticker_list(group)
        print(f"✅ '{group}' 그룹에서 {len(ticker_list)}개 티커 로드됨")
        return ticker_list
    except Exception as e:
        print(f"❌ 티커 리스트 로드 실패: {e}")
        return []
...
if __name__ == "__main__":
    ...
    # 방법 1: ticker_list.py에서 자동으로 로드
    print("\n" + "=" * 60)
    print("티커 리스트 로딩 옵션:")
    print("=" * 60)
    print("1. 전체 티커 (all)")
    print("2. 테스트용 (test)")
    print("3. 반도체 섹터 (semiconductor)")
    print("4. IT 섹터 (it)")
    print("5. 자동차 섹터 (auto)")
    print("6. 바이오 섹터 (bio)")
    print("7. 화학 섹터 (chemical)")
    print("8. 수동 입력")
    ...
    if choice in group_map:
        ticker_list = load_ticker_list(group_map[choice])
    elif choice == '8':
        # 수동 입력 예시
        ticker_list = [
        ...
        print(f"✅ 수동으로 {len(ticker_list)}개 티커 입력됨")
    else:
        print("잘못된 선택입니다. 테스트용으로 실행합니다.")
    ...
    if len(ticker_list) == 0:
        ...
        # 실행 확인
        print(f"\n{len(ticker_list)}개 티커를 처리하시겠습니까? (y/n): ", end='')
