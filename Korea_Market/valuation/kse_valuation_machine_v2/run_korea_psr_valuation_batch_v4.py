# -*- coding: utf-8 -*-
"""
Korea valuation batch runner v4
자동화된 재무제표 데이터 수집 적용
"""
...
# 새로운 자동화 모듈 임포트
from get_fs_data_by_ticker_v2 import extract_quarterly_revenue_auto, fetch_table_data
...
def process_single_ticker(
        ...
        db_info: dict,
        fs_df: Optional[pd.DataFrame] = None,  # 추가: fs_df 재사용
    ...
    try:
        ...
        # ========================================
        # 변경 사항: 자동화된 재무제표 수집 사용
        # ========================================
        print(f"[{ticker}] 재무제표 데이터 자동 수집 중...")
        df_rev = extract_quarterly_revenue_auto(
            db_info=db_info,
            ticker=ticker,
            fs_df=fs_df  # fs_df 재사용 (None이면 내부에서 자동 조회)
        )
        ...
        print(f"[{ticker}] ✓ 매출 데이터 {len(df_rev)}개 확보 (DataGuide + DART 결합)")
        # ========================================
...
def run_batch(group: str = "all", value_start_date: Optional[str] = None,
    ...
    개선사항:
    - fs_df를 한 번만 로드하여 모든 ticker에 재사용 (성능 최적화)
    - 자동화된 재무제표 수집으로 수동 업로드 불필요
    """
    ok, fail = 0, 0
    tickers = get_ticker_list(group=group)

    # ========================================
    # 성능 최적화: fs_df 사전 로드 (한 번만 로드)
    # ========================================
    print("\n" + "=" * 70)
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("🚀 Korea PSR Valuation Batch v4")
    print("   - 자동화된 재무제표 수집 (DataGuide + DART)")
    print("   - 수동 업로드 불필요")
    print("=" * 70 + "\n")
