...
from DATA.hs_code_map import HS_CODE_MAP
...
# CSV 저장 경로 설정
# ================================
CSV_EXPORT_DIR = r"C:\Users\82108\OneDrive\INVESTMENT\Data_Asnal\Export_data"


def save_long_df_to_csv(long_df: pd.DataFrame, hs_code: str) -> None:
    """
    long format 예측 결과를 HS Code별 CSV 파일로 저장합니다.
    파일명: forecast_{hs_code}_{YYYYMMDD}.csv

    Args:
        long_df: to_long_format() 반환 결과
        hs_code: 해당 HS Code 문자열
    """
    try:
        os.makedirs(CSV_EXPORT_DIR, exist_ok=True)
        date_str = datetime.now().strftime('%Y%m%d')
        filename = f"forecast_{hs_code}_{date_str}.csv"
        filepath = os.path.join(CSV_EXPORT_DIR, filename)
        long_df.to_csv(filepath, encoding='utf-8-sig')
        print(f"[CSV 저장] {filepath}")
    except Exception as e:
        print(f"[CSV 저장 오류] {hs_code}: {str(e)}")
...
def process_single_hscode(hs_code, db_info, horizon=24, mem_manager=None):
    ...
    try:
        ...
        # CSV 저장 (DB 저장 전 검증용)
        save_long_df_to_csv(long_df, hs_code)
...
def main(start_idx=None, end_idx=None):
    ...
    # HS Code 리스트 (HS_CODE_MAP 기반)
    hs_list = list(hs_code_list)  # 전역 리스트 복사
...
# 6. 실행
# ================================
PART_NUMBER = 3   # 이 파일의 파트 번호 (1, 2, 3 중 하나)
TOTAL_PARTS = 3   # 전체 파트 수

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='무역 데이터 예측 배치 프로세스 - Part 3/3')
    ...
    # --start/--end 미지정시 자동 구간 분배 (HS_CODE_MAP 기반)
    if args.start is None and args.end is None and args.range is None:
        total_count = len(hs_code_list)
        chunk = total_count // TOTAL_PARTS

        args.start = (PART_NUMBER - 1) * chunk
        args.end   = chunk if PART_NUMBER < TOTAL_PARTS else total_count
...
# 0번째부터 99번째까지 (100개)
# python run_trade_adv_forecast_v3_part3.py --range 0:100
# 시작만 지정 (끝은 리스트 끝까지)
# python run_trade_adv_forecast_v3_part3.py --start 0
# # 끝만 지정 (처음부터 지정 인덱스까지)
# python run_trade_adv_forecast_v3_part3.py --end 100
# # 시작과 끝 모두 지정
# python run_trade_adv_forecast_v3_part3.py --start 100 --end 200
# 터미널 1
# python run_trade_adv_forecast_v3_part1.py --range 0:167
#
# 터미널 2
# python run_trade_adv_forecast_v3_part2.py --range 167:334
#
# 터미널 3
# python run_trade_adv_forecast_v3_part3.py --range 334:500
