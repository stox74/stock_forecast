...
def forecast_sarima(
    ...
    if try_transforms:
        try:
            ...
            clear_memory()
            return {"error": f"SARIMA 적합 실패: {e}"}

    clear_memory()
    return {"error": "SARIMA 적합 실패"}
...
def forecast_ets(
    ...
    try:
        ...
        return {
            "forecast": fc,
            "spec": {"seasonal_periods": int(m), "trend": "add", "seasonal": "add" if m > 1 else None}
        }
    except Exception:
        pass
