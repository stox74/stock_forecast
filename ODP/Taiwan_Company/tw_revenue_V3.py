"""tw_revenue.py — 대만 대표기업 월 매출 수집·예측·시각화 (단일 파일 v5)

★ 사용법: 이 파일을 아무 폴더에나 두고 "더블클릭" → 메뉴에서 숫자 선택.
   DB(revenue.db)와 차트(output 폴더)는 이 파일과 같은 폴더에 생성됩니다.

v5 변경사항:
 - 대상 기업을 대만 시가총액 상위 50개로 확대
 - 예측 모델 4종: SARIMA / Theta / ETS / Ensemble(평균, 기본값)
 - 시각화 시 모델 선택 가능 + 4개 모델 비교 차트({코드}_models.png) 추가
 - 기존 DB는 자동으로 새 스키마로 이전됨 (기존 예측 데이터 유지)
"""
# --- 필요 패키지 자동 설치 (최초 1회) -------------------------------
...
# === forecaster.py ===
# ======================================================================
"""월 매출 예측: SARIMA / Theta / ETS / Ensemble 4종.

- 모든 모델은 로그 변환된 매출에 적합 후 지수 역변환
- Ensemble은 성공한 개별 모델 예측치의 단순 평균 (신뢰구간도 평균)
- 결과는 모델별로 forecast 테이블에 저장되어 시각화 시 선택 가능
"""
import warnings
...
def _fit_sarima(y, horizon):
    ...
    return (np.exp(fc.predicted_mean),
            np.exp(ci.iloc[:, 0]), np.exp(ci.iloc[:, 1]))


def _fit_ets(y, horizon):
    ...
    return (np.exp(sf["mean"]), np.exp(sf["pi_lower"]), np.exp(sf["pi_upper"]))


def _fit_theta(y, horizon):
    from statsmodels.tsa.forecasting.theta import ThetaModel
    model = ThetaModel(y, period=12)
    res = model.fit()
    mean = res.forecast(horizon)
    pi = res.prediction_intervals(horizon, alpha=0.05)
    return (np.exp(mean), np.exp(pi.iloc[:, 0]), np.exp(pi.iloc[:, 1]))


def forecast_company(conn, company_id: str, horizon: int = 6):
    """4개 모델로 예측하고 모두 DB에 저장. 저장된 행 리스트 반환."""
    ...
    # 결측월은 선형 보간 (모델 적합 안정성)
    y = np.log(df["revenue"].astype(float).interpolate(limit_direction="both"))

    basis_year, basis_month = int(rows[-1][0]), int(rows[-1][1])
    fitters = {"sarima": _fit_sarima, "theta": _fit_theta, "ets": _fit_ets}
    results = {}
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        for name, fn in fitters.items():
            try:
                results[name] = fn(y, horizon)
    ...
    # Ensemble: 성공한 모델들의 단순 평균
    means = pd.concat([r[0] for r in results.values()], axis=1).mean(axis=1)
    los = pd.concat([r[1] for r in results.values()], axis=1).mean(axis=1)
    his = pd.concat([r[2] for r in results.values()], axis=1).mean(axis=1)
    ...
    for model_name, (mean, lo, hi) in results.items():
        for ts in mean.index:
            out.append({
                ...
                "predicted": float(mean.loc[ts]),
                "lower_95": float(lo.loc[ts]),
                "upper_95": float(hi.loc[ts]),
            })
    upsert_forecast(conn, out)
    name = COMPANIES.get(company_id, company_id)
    ok = ", ".join(m for m in MODELS if m in results)
    print(f"[forecast] {name}({company_id}): basis={basis_year}-{basis_month:02d}, "
          f"{horizon}개월 x [{ok}] 저장")
    return out
