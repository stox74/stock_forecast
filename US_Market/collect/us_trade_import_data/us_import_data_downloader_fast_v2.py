...
# 4. 병렬 처리로 수입 데이터 수집
def get_us_import_data_parallel(hs_list, start='2013-01', end=None, api_key='your_key_here', max_workers=50):
    """
    ...
    - end: 종료 날짜 (YYYY-MM 형식). None이면 '오늘 - 2개월'을 자동 사용
           (Census API는 해당 월 말 기준 약 5주 후에 데이터를 발표하므로,
            너무 최근 달을 요청하면 항상 빈 값만 돌아옵니다)
    ...
    """
    if end is None:
        end = (pd.Timestamp.today() - pd.DateOffset(months=2)).strftime('%Y-%m')
        print(f"[자동] end 미지정 → 오늘 기준 최신 안전 월({end})을 사용합니다.")
...
if __name__ == "__main__":
    ...
    us_import_month, us_import_quarter = get_us_import_data_parallel(
        hs_list=hs_code,
        start='2015-01',
        end=None,           # None -> '오늘 - 2개월' 자동 계산 (과거처럼 특정 월에 고정하지 않음)
        api_key=api_key,
