...
def _ddl_revenue_forecast(table_name: str, with_collate: bool = True, collation: str = "utf8mb4_0900_ai_ci") -> str:
    """Revenue forecast 테이블 DDL - Long format (indicator/value 방식)"""
    tail = f"ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE={collation};" if with_collate else \
        "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"
    return f"""
    CREATE TABLE IF NOT EXISTS `{table_name}` (
      ...
      KEY `idx_created_at` (`created_at`),
      UNIQUE KEY `uq_ticker_date_indicator` (`ticker`, `date`, `indicator`)
    ) {tail}
    """
...
def upsert_revenue_forecast_to_db(df: pd.DataFrame,
    ...
    """
    Revenue forecast 데이터를 DB에 업서트 (Long format: indicator/value)
    - Wide format을 Long format으로 변환하여 저장
    - (ticker, date, indicator) 기준으로 덮어쓰기
    """
    if df is None or df.empty:
        return 0
    ...
    use_df = df.copy()
    if 'date' not in use_df.columns and 'date_month_end' in use_df.columns:
        use_df = use_df.rename(columns={'date_month_end': 'date'})
    elif 'date' in use_df.columns and 'date_month_end' in use_df.columns:
        use_df['date'] = pd.to_datetime(use_df['date'], errors='coerce')
        use_df['date'] = use_df['date'].fillna(pd.to_datetime(use_df['date_month_end'], errors='coerce'))
        use_df = use_df.drop(columns=['date_month_end'])
    ...
    # 3) 타입/중복 정리
    use_df['date'] = pd.to_datetime(use_df['date'], errors='coerce')
    use_df = use_df.dropna(subset=['ticker', 'date'])
    use_df = use_df.drop_duplicates(subset=['ticker', 'date'], keep='last')
    ...
    forecast_run_date = pd.Timestamp.utcnow().tz_localize(None).date()  # → DATE 형식
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable-audit", action='store_true', help="감사 기능 비활성화")
    ...
    log("TICKER-RANGE",
        f"Processing tickers [{start_idx}:{end_idx}] = {len(target_tickers)} tickers from total {len(ticker_list)}")

    # ═══════════════════════════════════════════════════════════════════════════
    # 감사 기능 실행 (옵션)
    # ═══════════════════════════════════════════════════════════════════════════
    if not args.disable_audit:
        log("AUDIT-START", "=== Ticker DB Coverage Audit ===")
        ...
        # 감사 기능 2: 첫 번째 티커로 Smoke Test
        if target_tickers:
            log("SMOKE-TEST-START", f"=== Smoke Test for {target_tickers[0]} ===")
            try:
                smoke_test_db_tables(db_info, target_tickers[0])
                log("SMOKE-TEST-OK", "Smoke test completed successfully")
            except Exception as e:
                log("SMOKE-TEST-ERR", f"Smoke test failed: {e}")

        # 감사 기능 3: 첫 번째 티커로 상세 진단
        if target_tickers:
            log("DIAG-START", f"=== Detailed Diagnosis for {target_tickers[0]} ===")
            try:
                diag_revenue_and_mcap_gaps(db_info, target_tickers[0])
                log("DIAG-OK", "Detailed diagnosis completed successfully")
            except Exception as e:
                log("DIAG-ERR", f"Detailed diagnosis failed: {e}")

        log("AUDIT-COMPLETE", "=== All Audit Functions Completed ===\n")
    else:
        log("AUDIT-SKIP", "Audit functions disabled (--disable-audit)")
...
# python US_company_valuation_exe_v3_final.py --start 0 --end 100
# python US_company_valuation_exe_v3_final.py --start 100 --end 200
# python US_company_valuation_exe_v3_final.py --start 0 --end 100 --batch-size 50
# python US_company_valuation_exe_v3_final.py --start 0 --end 100 --disable-audit
