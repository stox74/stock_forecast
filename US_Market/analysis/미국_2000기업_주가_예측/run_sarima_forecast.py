...
def main():
    ...
    """
    print("=" * 80)
    print("SARIMA 기반 미국 주식 월말 주가 예측")
    print("=" * 80)
    ...
    # 테스트 모드 설정
    print("\n실행 모드 선택:")
    print("1. 테스트 모드 (상위 10개 티커)")
    print("2. 테스트 모드 (상위 20개 티커)")
    print("3. 중간 규모 (상위 100개 티커)")
    print("4. 전체 실행 (2000개 티커)")

    mode = input("선택 (1, 2, 3 or 4): ").strip()

    if mode == '1':
        test_tickers = ticker_list[:10]
        print(f"\n[테스트 모드 - 10개] 처리 대상: {len(test_tickers)}개 티커")
        print(f"티커 목록: {test_tickers}")
    elif mode == '2':
        test_tickers = ticker_list[:20]
        print(f"\n[테스트 모드 - 20개] 처리 대상: {len(test_tickers)}개 티커")
        print(f"샘플 티커: {test_tickers[:10]}")
    elif mode == '3':
        test_tickers = ticker_list[:100]
        print(f"\n[중간 규모 - 100개] 처리 대상: {len(test_tickers)}개 티커")
        print(f"샘플 티커: {test_tickers[:10]}")
    elif mode == '4':
