...
def find_best_ets_params(
    ...
    --------
    Tuple[str, str, str, bool, float]
        (trend, seasonal, seasonal_type, damped_trend, best_aic)
    """
    best_aic = np.inf
    best_params = None
    ...
    for trend in trend_options:
        for seasonal in seasonal_options:
            for damped in damped_options:
                # damped_trend는 trend가 있을 때만 적용 가능
                if damped and trend is None:
                    continue

                # 곱셈 계절성은 양수 데이터만 가능
                if seasonal == 'mul' and (log_prices <= 0).any():
                    continue

                # 곱셈 추세는 양수 데이터만 가능
                if trend == 'mul' and (log_prices <= 0).any():
                    continue
    ...
    if best_params is None:
        # 최적화 실패 시 기본값
        return 'add', 'add', False, np.inf

    return (
        best_params['trend'],
        best_params['seasonal'],
        best_params['damped_trend'],
        best_params['aic']
    )


def forecast_ets_optimized(
        ticker: str,
    ...
    try:
        ...
        # 2. 최적 파라미터 탐색
        if optimize:
            trend, seasonal, damped, best_aic = find_best_ets_params(
                log_prices,
                seasonal_periods=seasonal_periods
            )
        else:
            # 기본 파라미터: additive trend, additive seasonal
            trend = 'add'
            seasonal = 'add'
            damped = False
            best_aic = None
        ...
        result = {
            ...
            'seasonal': seasonal,
            'damped_trend': damped,
            'aic': fitted_model.aic if hasattr(fitted_model, 'aic') else best_aic,
        ...
        print(f"Error in ETS forecast for {ticker}: {str(e)}")
        return None
    ...
    try:
        ...
        with connection.cursor() as cursor:
            for result in forecast_results:
                ...
                # 해당 티커의 오늘 날짜 ETS 예측 데이터 삭제 (덮어쓰기)
    ...
    try:
        for idx, ticker in enumerate(tickers, 1):
            ...
            try:
                ...
                # ETS 예측
                forecast_result = forecast_ets_optimized(
                    ticker,
                    df_monthly,
                    forecast_months,
                ...
                # 파라미터 정보 출력
                trend_str = forecast_result['trend'] if forecast_result['trend'] else 'None'
                seasonal_str = forecast_result['seasonal'] if forecast_result['seasonal'] else 'None'
                damped_str = 'D' if forecast_result['damped_trend'] else ''

                print(f"OK (Current: ${forecast_result['current_price']:.2f}, " +
                      f"6M Forecast: ${forecast_result['forecast_prices'][-1]:.2f}, " +
                      f"Model: {trend_str}/{seasonal_str}{damped_str})")
    ...
    try:
        ...
        if ticker and forecast_date:
            ...
            query = """
                    SELECT date, ticker, item, value, forecast_at
                    FROM us_stock_price_forecast_result
                    WHERE item = 'ets' AND forecast_at = %s
