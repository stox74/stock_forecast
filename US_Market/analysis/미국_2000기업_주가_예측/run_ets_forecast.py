...
from stock_price_forecast_ets import (
...
def main():
    ...
    print("예측 설정:")
    print("-" * 80)
    print("모델: ETS (Exponential Smoothing)")
    ...
    if OPTIMIZE:
        print("파라미터 최적화: 활성화")
        print("  - Trend: None, Additive, Multiplicative")
        print("  - Seasonal: None, Additive, Multiplicative")
        print("  - Damped Trend: True, False")
        print("  - 선택 기준: AIC 최소화")
    else:
        print("파라미터 최적화: 비활성화 (기본: Additive Trend + Additive Seasonal)")
    ...
    if success > 0:
        ...
        if not df_sample.empty:
            print(f"\n{sample_ticker} ETS 예측 결과 (예측일: {today}):")
            print(df_sample.to_string(index=False))
        else:
            # 가장 최근 예측 결과 조회
            df_sample = get_forecast_summary(ticker=sample_ticker)
            if not df_sample.empty:
                print(f"\n{sample_ticker} 최근 ETS 예측 결과:")
    ...
    if not df_summary.empty:
        print(f"\n총 {len(df_summary)}개 티커의 ETS 예측 데이터:")
